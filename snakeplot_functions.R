...
snakePlot <- function(ec1, tm1, ic1, tm2, ec2, tm3,
  ...
  cordDf <- data.frame(x=cord[,1], y=cord[,2])
  return(cordDf)
}
...
snake_angle_ggplot <- function(snakeplot_df, title="", min_angle=0, max_angle=80, direction = -1, mutated_size = FALSE){
  plot <- ggplot(snakeplot_df, aes(x = x, y= y, label=text, color = CA_CX_angle)) + 
    ggtitle(title) +
    geom_path(color = "black")
  ifelse(mutated_size,
         plot <- plot + geom_point(size = snakeplot_df$mutated*10, stroke = 0, alpha = snakeplot_df$mutated*0.95) +
           geom_text(hjust=0.5, vjust=0.5, color="white", size= snakeplot_df$mutated*7 ) ,
         plot <- plot + geom_point(size = 7, stroke = 0, alpha = 0.9) +
           geom_text(hjust=0.5, vjust=0.5, color="white", size= 6*0.75) 
  )
  plot <- plot + scale_color_viridis(name = 'angle °',
...
snake_distance_ggplot <- function(snakeplot_df, title="", min_distance=0, max_distance=5, direction = 1, mutated_size = FALSE){
  ...
  plot <- plot +
    ...
    plot <- plot + scale_color_viridis(name = 'RMSD (Å)', 
